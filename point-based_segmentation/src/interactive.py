...
def overlay_mask_on_bgr(image_bgr: np.ndarray, mask: np.ndarray, alpha: float = 0.45):
    """
    Overlay a segmentation mask on a BGR image.
...
def draw_points_on_bgr(image_bgr: np.ndarray, points, labels):
    """
    Draw foreground/background points on an image.

    Args:
        image_bgr (np.ndarray): Input image
        points (list): List of points
        labels (list): Corresponding labels (1=foreground, 0=background)

    Returns:
        np.ndarray: Image with points visualized
    """
    ...
    for (x, y), label in zip(points, labels):
        ...
        text = f"({x},{y})"
        cv2.putText(canvas, text, (x + 8, max(20, y - 8)),
    ...
    cv2.putText(canvas, info1, (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 3)
    cv2.putText(canvas, info1, (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
    cv2.putText(canvas, info2, (15, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 3)
    cv2.putText(canvas, info2, (15, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
...
class InteractiveSegmenter:
    """
    Interactive SAM segmentation tool using mouse input.
    ...
    def __init__(self, config: dict):
        ...
        print(f"[2/4] Loading image... ({self.image_path})")
        self.image = load_image(self.image_path)

        if self.resize_enabled:
            print(f"[2-1/4] Applying resize (long side -> {self.resize_size})")
            self.image, self.scale = resize_image(self.image, self.resize_size)
        else:
            self.scale = 1.0

        print(f"[3/4] Preparing display image... (device={self.device})")
        ...
        print("[4/4] Ready")
        print("Left click: FG | Right click: BG | Enter: Run | r: Reset | s: Save | q: Quit")
    ...
    def run_segmentation(self):
        """Run SAM segmentation with current points."""
        ...
        overlay_bgr = overlay_mask_on_bgr(self.image_bgr, result["mask"])
        cv2.imshow(RESULT_WINDOW_NAME, overlay_bgr)

        print(f"Done - score: {result['score']:.4f}")
    ...
    def run(self):
        """Start interactive segmentation loop."""
...
def run_interactive(config: dict):
    """
    Entry function to run interactive segmentation.
    """
